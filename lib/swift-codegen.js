 ...
 */
exports.swiftModels = function generateServices (app, appName, modelPrefix, modelPostfix, includeMethodPath = null, executeMethodPath = null, verbose) {
...
function describeModels (app) {
  ...
  app.handler('rest').adapter.getClasses().forEach(function (c) {
    ...
    c.methods.forEach(function fixArgsOfPrototypeMethods (method, idx, array) {
      // TODO: Remove method here

      // Check include first
      if (enableInclude &&
        Array.isArray(modelIncludeMethods) &&
        modelIncludeMethods.indexOf(method.fullName.replace('.prototype', '')) === -1) {
        skipIdx.push(idx);
        return;
      }
      // Next check execute
      if (enableExecute &&
        ...
        modelExecuteMethods.indexOf(method.fullName.replace('.prototype', '')) !== -1) {
        skipIdx.push(idx);
        return;
      }
    ...
    for (let i = skipIdx.length - 1; i >= 0; i--) {
      c.methods.splice(skipIdx[i], 1);
    }
    result[name] = c;
  });
  ...
  listFiles.forEach(function (file) {
    ...
    try {
      ...
      jsonInclude.include.forEach(function (includeName) {
        includeMethod.push(`${fileName}.${includeName}`);
      });

      if (includeMethod.length > 0) {
        includeMethods[fileName] = includeMethod;
        if (!enableInclude) {
          enableInclude = true;
        }
      }
...
function processingExecuteMethod (executePath) {
  ...
  listFiles.forEach(function (file) {
    ...
    try {
      ...
      const executeMethod = [];
      const includeMethod = includeMethods[fileName] ? includeMethods[fileName] : [];
      jsonExecute.execute.forEach(function (executeName) {
        if (includeMethod.indexOf(`${fileName}.${executeName}`) === -1) {
          executeMethod.push(`${fileName}.${executeName}`);
        }
      });

      if (executeMethod.length > 0) {
        executeMethods[fileName] = executeMethod;
        if (!enableExecute) {
          enableExecute = true;
        }
      }
