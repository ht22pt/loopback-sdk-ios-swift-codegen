...
function addSwiftNames (models, modelPrefix, modelPostfix, verbose) {
  for (let modelName in models) {
    ...
    meta.swiftModelName = modelPrefix + pascalCase(modelName) + modelPostfix;
    meta.swiftRepoName = modelPrefix + pascalCase(modelName) + 'Repository';
...
function addSwiftMethodInfo (meta, method, modelName, skipOptionalArguments) {
  ...
  method.accepts.forEach(function (param) {
    ...
    let argType = convertToSwiftArgType(param.type, param.arg, swiftModelType);
  ...
  const swiftReturnType = convertToSwiftReturnType(returnType, modelName, meta.swiftModelName);
...
function convertToSwiftArgType (type, name, swiftModelType) {
  let swiftType = argTypeConversionTable[type + ' ' + name];
  swiftType = swiftType || argTypeConversionTable[type];
  if (swiftType) {
    swiftType = swiftType.replace('<swiftModelType>', swiftModelType);
  }
  return swiftType;
}

function convertToSwiftReturnType (type, modelName, swiftModelName) {
  if (type === modelName) {
    return swiftModelName;
  }
