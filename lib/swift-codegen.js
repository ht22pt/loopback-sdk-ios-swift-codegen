...
let methodNamesUserToKeep = [
  // The followings are methods need to create and delete.
  //'create'
  'replaceOrCreate'
];
...
function addObjCNames(models, modelPrefix, verbose) {
  for (let modelName in models) {
    ...
    if (meta.baseModel === 'Model' || meta.baseModel === 'PersistedModel' || baseModels.indexOf(meta.baseModel) !== -1) {
      meta.objcBaseModel = 'LB' + meta.baseModel;
    } else {
      throw new Error('Unknown base model: '
        ' + meta.baseModel + '
        ' for model: '
        ' + modelName + '
        '');
    ...
    for (let propName in meta.props) {
      if (excutePropeties.indexOf(propName) !== -1) {
  ...
  method.accepts.forEach(function (param) {
        ...
        if (param.http && param.http.source === 'body') {
          if (bodyParamAssignments) {
            throw new Error(
              'Multiple body arguments specified in method: '
              ' + method.name +
              '' of model: '' + modelName + ''
              ');
            }
